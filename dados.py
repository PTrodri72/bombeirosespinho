...
import os
from dados import DB, hash_password, check_password
...
@app.route('/')
def home():
    content = '<h1 class="text-4xl font-black text-white">Bem-vindo ao Portal de Comando</h1>'
    return render_template_string(LAYOUT, content=content)

@app.route('/login_page')
def login_page():
    content = """
    <form method='POST' action='/login' class='glass p-8 rounded-xl max-w-md mx-auto'>
        <input name='email' type='email' placeholder='Email' class='w-full p-2 mb-4 rounded' required>
        <input name='senha' type='password' placeholder='Senha' class='w-full p-2 mb-4 rounded' required>
        <button type='submit' class='w-full p-2 bg-red-600 text-white rounded'>Login</button>
    </form>
    """
    return render_template_string(LAYOUT, content=content)
...
@login_required
def dashboard():
    from dados import gerar_dashboard_html
